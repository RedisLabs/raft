...
void raft_set_snapshot_metadata(raft_server_t *me,
                                raft_term_t term, raft_index_t idx)
{
    me->last_applied_idx = idx;
...
/* Returns targeted node_id if leadership transfer is in progress
 * otherwise returns RAFT_NODE_ID_NONE
 */
...
/* Forces the raft server to invoke an election on the next raft_periodic() call */
...
raft_node_t *raft_add_node_internal(raft_server_t *me, raft_entry_t *ety,
                                    void *udata, raft_node_id_t id, int is_self)
{
    ...
    if (me->cb.notify_membership_event) {
        me->cb.notify_membership_event(me, raft_get_udata(me),
                                       node, ety, RAFT_MEMBERSHIP_ADD);
    }

    return node;
}

static raft_node_t *raft_add_non_voting_node_internal(raft_server_t *me,
                                                      raft_entry_t *ety,
                                                      void *udata,
                                                      raft_node_id_t id,
                                                      int is_self)
...
raft_node_t *raft_add_non_voting_node(raft_server_t *me, void *udata,
                                      raft_node_id_t id, int is_self)
{
...
{
    if (me->cb.notify_membership_event) {
        me->cb.notify_membership_event(me, raft_get_udata(me), node, NULL,
    ...
    size_t size = sizeof(*me->nodes) * (me->num_nodes - i - 1);
...
void raft_handle_append_cfg_change(raft_server_t *me,
        ...
        case RAFT_LOGTYPE_ADD_NONVOTING_NODE:
            if (!is_self) {
                if (node && !raft_node_is_active(node)) {
                    ...
                    node = raft_add_non_voting_node_internal(me, ety, NULL,
                                                             node_id, is_self);
                    assert(node);
                }
            }
...
void raft_handle_remove_cfg_change(raft_server_t *me,
                                   raft_entry_t *ety, const raft_index_t idx)
{
    if (!raft_entry_is_cfg_change(ety)) {
        return;
    }
...
int raft_periodic(raft_server_t *me, int msec_since_last_period)
{
    ...
    /* Needs to be outside state check as the node can become a follower and
     * still timeout */
                                     ...
                                     msg_appendentries_response_t *r)
{
     ...
     * even if it can't accept the append_entry. */
    ...
    if (r->current_idx == raft_get_current_idx(me) &&
        raft_get_transfer_leader(me) == raft_node_get_id(node) &&
        me->cb.send_timeoutnow && !me->sent_timeout_now) {
...
static int raft_receive_term(raft_server_t *me, raft_term_t term)
{
    ...
    if (!raft_is_follower(me) && me->current_term == term) {
        raft_become_follower(me);
    }
                          ...
                          msg_requestvote_response_t *r)
{
    ...
    if (!vr->prevote && vr->term > raft_get_current_term(me)) {
    ...
    if (me->current_term == vr->term &&
        (me->voted_for != RAFT_NODE_ID_NONE && me->voted_for != vr->candidate_id)) {
        goto done;
    }
    ...
    raft_log_node(me, vr->candidate_id,
                  "node requested vote: %d, t:%ld, pv:%d replying: %s",
                  vr->candidate_id,
                  vr->term,
                  vr->prevote,
                                   ...
                                   msg_requestvote_response_t *r)
{
    ...
    if (r->prevote) {
        ...
        if (!raft_is_precandidate(me) ||
            r->request_term != me->current_term + 1) {
            return 0;
        }
    } else {
...
int raft_apply_entry(raft_server_t *me)
{
    ...
    node_id = me->cb.get_node_id(me, raft_get_udata(me), ety, log_idx);
    node = raft_get_node(me, node_id);
...
raft_entry_t **raft_get_entries_from_idx(raft_server_t *me,
                                         raft_index_t idx, int *n_etys)
{
    if (idx > raft_get_current_idx(me)) {
        *n_etys = 0;
        return NULL;
    }
...
{
    return ety->type == RAFT_LOGTYPE_ADD_NODE ||
...
{
    assert(raft_get_current_idx(me) > 0);
...
{
    raft_entry_t *ety = raft_calloc(1, sizeof(*ety) + data_len);
...
int raft_queue_read_request(raft_server_t *me,
                            func_read_request_callback_f cb, void *cb_arg)
{
 ...
 * timeout = how long this should be allowed to take in milliseconds,
 *           as calculated by calls to raft_periodic().
 * Returns an error if leadership transfer is already in progress or the
 * targeted node_id is unknown.
 */
 ...
 * Internally used because either we have timed out our attempt or because we
...
void raft_reset_transfer_leader(raft_server_t *me, int timed_out)
{
    ...
    if (me->node_transferring_leader_to != RAFT_NODE_ID_NONE) {
        if (me->cb.notify_transfer_event) {
            ...
            me->cb.notify_transfer_event(me, raft_get_udata(me), result);
        }
